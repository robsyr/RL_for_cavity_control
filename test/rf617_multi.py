...
n_bins = 5  # Number of 'sampled' Gaussians
...
def morphing(setting, n_dimensions):
    ...
    binning = [ROOT.RooBinning(n_bins, 0.0, 4.0) for dim in range(n_dimensions)]
    ...
    # Uncomment to see input plots for the first dimension (you mightn need to increase the morphed samples)
    # f1 = x_vars[0].frame()
...
c1 = ROOT.TCanvas()
frame = mu_vars[0].frame(
    Title="Learned vs analytical summed negative logarithmic Likelihood",
    Range=(mu_vars[0].getMin(), mu_vars[0].getMax()),
)
nll_gauss.plotOn(frame, ShiftToZero=True, LineColor="g", Name="gauss")
nll_morph.plotOn(frame, ShiftToZero=True, LineColor="c", Name="morph")
nllr_learned.plotOn(frame, LineColor="r", ShiftToZero=True, LineStyle="--", Name="learned")
frame.Draw()
...
lhr_learned.plotOn(frame_x, LineColor="r", Name="learned")
lhr_calc_final.plotOn(frame_x, LineColor="c", Name="analytical")
frame_x.Draw()
