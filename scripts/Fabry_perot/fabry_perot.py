...
class Environment(gym.Env):
    ...
    def calculate_power_noise(self, delta_L: float, noise: float) -> float:
        ...
        power_noise = (
            power_output_transmitted_fabry_perot(delta_L + noise + self.real)
            / power_equilibrium
        )
    ...
    def compute_reward(self, power_noise: float) -> int:
        ...
        reward = -np.abs(1.0 - power_noise) * 10

        if np.abs(power_noise) < 0.995:
    ...
    def step(self, action: np.ndarray):
        ...
        self.update_history(self.delta_L, power_noise)
    ...
    def update_history(self, delta_L: float, power_noise: float):
        """
        Update the history buffer with current delta_L and power noise.
        """
        self.history.append(delta_L)
