...
from fabry_perot_new_env_adapted import Environment
    ...
    for length in history_length:
        for level in noise_level:
            for size in action_size:
                ...
                models_dir = f"models_new_env_adapted/PPO_length_{length}_level_{level}_action_size_{size}_try_{i}/"
                logdir = f"logs_new_env_adapted/PPO_length_{length}_level_{level}_action_size_{size}_try_{i}/"
                ...
                model = PPO('MlpPolicy', env, verbose=1, learning_rate=0.01, tensorboard_log=f'{logdir}_tensorboard')
