...
TIMESTEPS_PER_SAVE = 500000  # Save every 500,000 steps instead of every 10,000
    ...
    for length in history_length:
        for level in noise_level:
            for size in action_size:
                ...
                models_dir = f"models/PPO_length_{length}_level_{level}_action_size_{size}_try_{i}/"
                logdir = f"logs/PPO_length_{length}_level_{level}_action_size_{size}_try_{i}/"
