...
class Environment(gym.Env):
    ...
    def __init__(self, history_length, noise_level):
    ...
    def step(self, action):
        ...
        reward = 0
        self.timestep += 1
        self.delta_L = 0.0009 * wavelength * action[
        ...
        power_noise = power_output_transmitted_fabry_perot(
            self.delta_L + noise_ + self.length_difference) / power_equilibrium  # length difference as offset

        #difference_per_step = self.delta_L + noise_ + self.length_difference

        self.length_difference += self.delta_L + noise_
        #self.length_difference = difference_per_step
